...
import { checkConnection } from '@wallet/scripts/apiActions'
import {
    copyToClipboard, currentConnectionKey, getBlurredAddress, limitNumberString,
} from '@wallet/scripts/utils'
import { LAMPORTS_PER_SOL } from '@solana/web3.js'
import { ButtonCopy, ButtonWalletClose } from '@wallet/components/form-elements/Buttons'
    ...
    return !hide && (
        <div className="card fs-2">
            ...
            <div className="d-grid my-1 px-2" style={{ gridTemplateColumns: "1fr auto" }}>
                <div className={clsx('text-end fw-bold', isSpending ? 'text-dark-red' : 'text-success')}>
                    {hideBalances ? '...' : limitNumberString(sum)}
                </div>
                <div className="text-start"><span className="fst-italic mx-2 fs-5">{t('currencySEV')}</span></div>
                <div className={clsx('text-end fw-bold', isSpending ? 'text-dark-red' : 'text-success')}>
                    {hideBalances ? '...' : sumUsd.toFixed(2)}
                </div>
                <div className="text-start"><span className="fst-italic mx-2 fs-5">{t('currencyUSD')}</span></div>
            </div>
        </div>
    )
}
