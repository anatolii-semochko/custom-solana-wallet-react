...
import { closeWallet } from '@js/wallet' // TODO - outer import !!!
import {
...
export const ButtonWalletClose = () => (
    <button className="btn-close ms-auto" aria-label={t('close')} onClick={() => closeWallet()} />
)
...
export const ButtonSendCoins = ({onClick, className}) => (
    <WalletButton
        label={<>{t('send')} <span className="fst-italic">{t('currencySEV')}</span></>}
        onClick={onClick}
        className={clsx('btn-primary w-100', className)}
