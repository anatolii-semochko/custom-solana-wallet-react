...
import { reloadAllWallets, setProviderUrl } from '@wallet/scripts/apiActions'
import { getWalletStateProperty, setWalletStateProperty } from '@wallet/scripts/storageActions'
import { setTranslations } from '@wallet/translations/translations'
...
const WalletContextProvider = ({ children }) => {
    ...
    const handleReloadRequest = (event) => {
        if (password && walletReload) {
            setTimeout(() => {
                walletReload().catch()
            }, config.RELOAD_AFTER_CHANGES_SECONDS * 1000)
        }
    }
