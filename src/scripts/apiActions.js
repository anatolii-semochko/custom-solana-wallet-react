...
import {
    readEncryptedWallets,
    writeEncryptedWallets,
    getCachedTokenData,
    setCachedTokenData,
} from '@wallet/scripts/storageActions'
...
export const fetchWalletTokensWithData = async (pubKeyStr) => {
    ...
    for (const t of tokens) {
        const cachedData = await getCachedTokenData(t.mint)
        if (cachedData) {
            t.data = cachedData
        } else {
            const tokenData = await getSevensTokenData(t.mint)
            t.data = tokenData ?? {}
            if (tokenData) {
                await setCachedTokenData(t.mint, tokenData)
            }
        }
    }
...
export const getSevensTokenDataWithCache = async (tokenPublicKey) => {
    try {
        const cachedData = await getCachedTokenData(tokenPublicKey)
        if (cachedData) {
            return cachedData
        }
        const tokenData = await getSevensTokenData(tokenPublicKey)
        if (tokenData) {
            await setCachedTokenData(tokenPublicKey, tokenData)
        }
...
export const burnSevensToken = async (tokenPublicKey, wallet) => {
    try {
        const tokenData = await getSevensTokenDataWithCache(tokenPublicKey)
        const mint = new PublicKey(tokenPublicKey)
