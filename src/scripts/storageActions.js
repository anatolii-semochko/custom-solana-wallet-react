...
const STORAGE_WALLET_TOKENS = config.STORAGE_WALLET_TOKENS_KEY
...
export const clearWallet = () => {
    if (hasChromeStorage) {
        ...
        chromeApi.storage.local.remove([STORAGE_WALLET_TOKENS])
        return
    }
    localStorage.removeItem(STORAGE_KEY)
    localStorage.removeItem(STORAGE_WALLET_STATE)
    localStorage.removeItem(STORAGE_WALLET_TOKENS)
}
...
export const getTokensCache = async () => {
    ...
    if (hasChromeStorage) {
        encoded = await new Promise((resolve) =>
            chromeApi.storage.local.get([STORAGE_WALLET_TOKENS], (res) =>
                resolve(res[STORAGE_WALLET_TOKENS] || null)
            )
        )
    } else {
        encoded = localStorage.getItem(STORAGE_WALLET_TOKENS)
    }
...
export const setTokensCache = async (tokensCache) => {
    const encoded = encode(tokensCache)
    if (hasChromeStorage) {
        return new Promise((resolve) =>
            chromeApi.storage.local.set({ [STORAGE_WALLET_TOKENS]: encoded }, () => resolve())
        )
    }
    localStorage.setItem(STORAGE_WALLET_TOKENS, encoded)
}

export const getCachedTokenData = async (tokenPublicKey) => {
    const cache = await getTokensCache()
    return cache[tokenPublicKey] || null
}

export const setCachedTokenData = async (tokenPublicKey, tokenData) => {
    const cache = await getTokensCache()
    cache[tokenPublicKey] = tokenData
    await setTokensCache(cache)
}
